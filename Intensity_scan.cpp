...
void Intensity_scan() {
  ...
  outputfilename.Form("$(MEG2SYS)/analyzer/macros/xec/sLED/gaindata/gain_%d-%d_%d.csv",runarray[0],runarray[runnum-1],rangemode);
  //string outputfilename="gain_"+std::to_string(runarray[0])+"-"+std::to_string(runarray[runnum-1])+"_"+std::to_string(rangemode)+".root";
  //TString outputfilepath=outputpath+outputfilename;
  ofstream output;
